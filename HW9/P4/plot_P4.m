...
[iter_sd,aval_sd,dxval_sd,xval_sd]=Rosenbrock_line_search(1);
[iter_newton,aval_newton,dxval_newton,xval_newton]=Rosenbrock_line_search(2);
[iter_BFGS,aval_BFGS,dxval_BFGS,xval_BFGS]=Rosenbrock_BFGS(3);
[iter_FRCG,aval_FRCG,dxval_FRCG,xval_FRCG]=Rosenbrock_FRCG(3);
[iter_PRCG,aval_PRCG,dxval_PRCG,xval_PRCG]=Rosenbrock_PRCG(3);
...
hold on;
grid on;
plot(0:(iter_sd-1),aval_sd(1:iter_sd),0:(iter_newton-1),aval_newton(1:iter_newton), ...
    0:(iter_BFGS-1),aval_BFGS(1:iter_BFGS),0:(iter_FRCG-1),aval_FRCG(1:iter_FRCG), ...
    0:(iter_PRCG-1),aval_PRCG(1:iter_PRCG),'Linewidth',2);
legend('SD','Newton','BFGS','FRCG','PRCG');
...
hold on;
grid on;
plot(0:(iter_sd-1),dxval_sd(1:iter_sd),0:(iter_newton-1),dxval_newton(1:iter_newton), ...
    0:(iter_BFGS-1),dxval_BFGS(1:iter_BFGS),0:(iter_FRCG-1),dxval_FRCG(1:iter_FRCG), ...
    0:(iter_PRCG-1),dxval_PRCG(1:iter_PRCG),'Linewidth',2);
legend('SD','Newton','BFGS','FRCG','PRCG');
set(gca,'Fontsize',fsz);
set(gca, 'YScale', 'log');
title('Initial configuration=[-1.2,1.0], tol=1e^-6','FontSize',fsz)
...
plot(xval_BFGS(1,:),xval_BFGS(2,:),'y-','LineWidth',2);
plot(xval_FRCG(1,:),xval_FRCG(2,:),'m-','LineWidth',2);
plot(xval_PRCG(1,:),xval_PRCG(2,:),'k-','LineWidth',2)
legend('contour','SD','Newton','BFGS','FRCG','PRCG');

hold off
title('Trajectory of initial configuration=[1.2,1.2], tol=1e^-6','FontSize',fsz)
