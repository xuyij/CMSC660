function plot_P3(initial_condition)
fsz = 20;
Na = 7; % the number of atoms
rstar = 2^(1/6);
init_config=initial_configuration(initial_condition,Na,rstar);
[iter_sd,fval_sd,gval_sd]=LJ_line_search(init_config,1);
[iter_newton,fval_newton,gval_newton]=LJ_line_search(init_config,2);
[iter_BFGS,fval_BFGS,gval_BFGS]=LJ_BFGS(init_config,3);
[iter_FRCG,fval_FRCG,gval_FRCG]=LJ_FRCG(init_config,3);
[iter_PRCG,fval_PRCG,gval_PRCG]=LJ_PRCG(init_config,3);
...
hold on;
grid on;
plot(0:(iter_sd-1),fval_sd(1:iter_sd),0:(iter_newton-1),fval_newton(1:iter_newton), ...
    0:(iter_BFGS-1),fval_BFGS(1:iter_BFGS),0:(iter_FRCG-1),fval_FRCG(1:iter_FRCG), ...
    0:(iter_PRCG-1),fval_PRCG(1:iter_PRCG),'Linewidth',2);
...
hold on;
grid on;
plot(0:(iter_sd-1),gval_sd(1:iter_sd),0:(iter_newton-1),gval_newton(1:iter_newton), ...
    0:(iter_BFGS-1),gval_BFGS(1:iter_BFGS),0:(iter_FRCG-1),gval_FRCG(1:iter_FRCG), ...
    0:(iter_PRCG-1),gval_PRCG(1:iter_PRCG),'Linewidth',2);
legend('SD','Newton','BFGS','FRCG','PRCG');
set(gca,'Fontsize',fsz);
title('Initial configuration=0, tol=1e^-6','FontSize',fsz)
xlabel('Iteration #','FontSize',fsz);
